...
```bash
# Basic scrape (runs in headless mode and lists image links)
npx github:mfadi1/gmaps-image-scraper --url "https://maps.app.goo.gl/dsyyKTns3o9JXENJ9"

# Scrape and download images locally to './scraped_images/'
npx github:mfadi1/gmaps-image-scraper --url "https://maps.app.goo.gl/dsyyKTns3o9JXENJ9" --download

# Scrape and limit search results (e.g., up to 20 images)
npx github:mfadi1/gmaps-image-scraper --url "https://maps.app.goo.gl/dsyyKTns3o9JXENJ9" --limit 20 --download
```
...
4. Run the scraper locally:
   ```bash
   npm run scrape -- --url "https://maps.app.goo.gl/dsyyKTns3o9JXENJ9"
   ```
...
| --- | --- | --- | --- |
| `--url` | Direct Google Maps short link (`maps.app.goo.gl`) or full URL. | **Yes** | - |
...
| `--output` | Directory to save downloaded images. | No | `./scraped_images/` |
...
async function run() {
  const mapsUrl = "https://maps.app.goo.gl/dsyyKTns3o9JXENJ9";
  const imgbbApiKey = process.env.IMGBB_API_KEY; // optional
  const imageLimit = 50;

  // returns an array of direct high-resolution image URLs
  const photos = await scrapePhotos(mapsUrl, imgbbApiKey, imageLimit);
